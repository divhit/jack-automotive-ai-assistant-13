          ...
          onClick={() => handleTileClick("In Progress Leads", 
            <div className="space-y-4">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.scriptProgress.currentStep === "screening").length}</span> leads in Screening stage</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.scriptProgress.currentStep === "qualification").length}</span> leads in Qualification stage</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.nextAction.isAutomated).length}</span> leads in automated follow-up sequences</li>
              </ul>
        ...
        <Card className="cursor-pointer hover:bg-gray-50 transition-colors"
          onClick={() => handleTileClick("Not Ready Leads", 
            <div className="space-y-4">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.creditProfile?.knownIssues.includes("Multiple Collections")).length}</span> with major collections issues</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.creditProfile?.knownIssues.includes("Recent Bankruptcy")).length}</span> with recent bankruptcies</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.sentiment === "Ghosted").length}</span> leads have gone silent (no response in 72+ hours)</li>
              </ul>
        ...
        <Card className="cursor-pointer hover:bg-gray-50 transition-colors"
          onClick={() => handleTileClick("Needs Action Leads", 
            <div className="space-y-4">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.nextAction.isOverdue && l.sentiment === "Frustrated").length}</span> overdue and showing frustration</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.nextAction.isOverdue && l.chaseStatus === "Manual Review").length}</span> flagged for manual review</li>
                <li><span className="font-semibold">{subprimeLeads.filter(l => l.nextAction.isOverdue && l.fundingReadiness === "Partial").length}</span> in progress leads with missed follow-ups</li>
              </ul>
