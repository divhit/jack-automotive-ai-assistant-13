-- ElevenLabs MCP Analytics Database Schema - FIXED VERSION
-- Extends existing CRM schema with advanced conversation analytics
-- FIXES: All JSONB casting errors corrected
...
-- Enhanced lead scoring function using ElevenLabs analytics - FIXED JSONB CASTING
...
BEGIN
  ...
  -- Calculate conversation analytics score - FIXED: Use jsonb_array_length instead of casting
  SELECT COALESCE(AVG(
...
-- Performance insights view - FIXED JSONB CASTING
...
SELECT 
  ...
  -- Buying signals and objections - FIXED: Use jsonb_array_length
